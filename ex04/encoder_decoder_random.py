...
cluster_size = 20
def compute_clusters(size):
    clusters = []
    for j in range(14):
        clusters.append(np.array([pics for (i,pics) in enumerate(X_lat) if Y_lat[i] == j]))
    for i in range(14):
        clusters[i] = clusters[i][:size]
    clusters = np.array(clusters)
    return clusters

clusters = compute_clusters(cluster_size)
...
lat_actual = tf.gather(clusters, Y)

loss = tf.reduce_mean(tf.square(lat_actual - tf.reshape(lat_pred, (-1, 1, 28, 28))))
...
def train():
    with tf.Session() as sess:
        ...
        for i in range(20000):
            ...
            if i % 2000 == 0 and i > 0:
                saver.save(sess, "/Users/sigi/PycharmProjects/ex04/weights_encoder_decoder_random.ckpt")
    ...
    with tf.Session() as sess:
        ...
        for i in range(20000):
            ...
            if i % 500 == 0:
                ...
                print('step %d, training accuracy %g, test accuracy %g' % (i, train_accuracy, test_accuracy))
            if i % 2000 == 0 and i > 0:
                saver.save(sess, "/Users/sigi/PycharmProjects/ex04/weights_encoder_decoder_latent.ckpt")
        test_accuracy = loss.eval(feed_dict={X: X_test, Y: Y_test, phase_train: False, keep_prob_middle: 1.0})
        print('test accuracy %s' % test_accuracy)
        train_accuracy = 0.0
        for i in range(int(len(X_train) / 1000)):
