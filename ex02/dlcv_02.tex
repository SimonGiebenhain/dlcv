...
\begin{document}
    \begin{ukon-infie}[3.12.17]{2}
		...
		\begin{exercise}[p=55]{}
			...
			We see that while the train accuracy hardly changes, the test accuracy deteriorates more severely. Therefore we conclude, that removing one layer causes the model to overfit, because it loses some ability to generalize.
			}
			\question{}{
			...
			Further decreasing of the step size, causes to model to learn more slowly. Furthermore the models cannot jump accross local minima as good as before, which causes them to get stuck in worse local minima than before.
