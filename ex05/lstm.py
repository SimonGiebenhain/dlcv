...
num_hidden = 96 # hidden layer num of features
...
def RNN(x, weights, biases):
    ...
    b_fc = tf.get_variable('b_fc', initializer=tf.constant(0.1, tf.float32, [7*17*16]))
...
with tf.Session() as sess:
    ...
    for step in range(1, training_steps+1):
        batch_x, batch_y = next_batch(train_x,train_y,timesteps,batch_size)
        ...
        if step % display_step == 0 or step == 1:
            ...
            print("Step " + str(step) + ", Minibatch Loss= " + \
                  ...
                  "{:.3f}".format(acc))
            test_x, test_y = next_test_batch(x[0:5], labels[0:5],28 , 50)
